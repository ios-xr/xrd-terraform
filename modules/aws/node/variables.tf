...
variable "hugepages_gb" {
  description = <<-EOT
  Number of 1GiB hugepages to allocate.  This is ignored if not using an XRd AMI generated by Packer.
  Default is to choose a number appropriate for the instance type.
  EOT
  type = number
  default = null
}
...
variable "isolated_cores" {
  description = <<-EOT
  CPU cores to isolate.  This should be an inclusive range: "<start-number>-<end-number>".  This is ignored if not using an XRd AMI generated by Packer.
  Default is to choose a range appropriate for the instance type.
  EOT
  type = string
  default = null
}
